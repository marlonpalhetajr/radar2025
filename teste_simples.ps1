# Script PowerShell para converter arquivo Excel para HTML - TESTE
...
$baseDir = "c:\Users\marlon.junior\OneDrive - Fapespa\radar2024"
$pastaExcel = Join-Path $baseDir "tabelas-excel\1 Pará"
$pastaHTML = Join-Path $baseDir "tabelas\1-para"

# Busca os arquivos dinamicamente
$arquivoExcel = (Get-ChildItem -Path $pastaExcel -Filter "Tabela 1*.xlsx" | Select-Object -First 1).FullName
$arquivoHTML = (Get-ChildItem -Path $pastaHTML -Filter "tabela-1*.htm" | Select-Object -First 1).FullName

if (-not (Test-Path $arquivoExcel)) {
    Write-Host "ERRO: Arquivo Excel nao encontrado" -ForegroundColor Red
    Write-Host $arquivoExcel -ForegroundColor Yellow
    exit
}

if (-not (Test-Path $arquivoHTML)) {
    Write-Host "ERRO: Arquivo HTML nao encontrado" -ForegroundColor Red
    Write-Host $arquivoHTML -ForegroundColor Yellow
    exit
}

Write-Host "`nArquivo Excel:" -ForegroundColor Yellow
Write-Host "  $arquivoExcel" -ForegroundColor White

Write-Host "`nArquivo HTML (sera atualizado):" -ForegroundColor Yellow
Write-Host "  $arquivoHTML" -ForegroundColor White
...
try {
    ...
    $workbook.SaveAs($arquivoHTML, 44)
    ...
    Write-Host "  $arquivoHTML" -ForegroundColor Cyan
